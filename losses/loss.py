...
# 自适应权重调整的损失组合
# 在计算总损失时，根据不同损失项在不同训练阶段或数据样本上的重要性，动态地调整它们的权重。
# 使用一个可学习的参数来乘以每个损失项，这个可学习参数在训练过程中根据梯度下降进行更新。
reprojection_weight = paddle.create_parameter(shape=[1], dtype='float32', default_initializer=paddle.nn.initializer.Constant(1.0))
depth_hint_weight = paddle.create_parameter(shape=[1], dtype='float32', default_initializer=paddle.nn.initializer.Constant(1.0))
smoothness_weight = paddle.create_parameter(shape=[1], dtype='float32', default_initializer=paddle.nn.initializer.Constant(1.0))
# ssim_weight = paddle.create_parameter(shape=[1], dtype='float32', default_initializer=paddle.nn.initializer.Constant(0.85))
# l1_weight = paddle.create_parameter(shape=[1], dtype='float32', default_initializer=paddle.nn.initializer.Constant(0.15))
...
def compute_losses(opt, inputs, outputs):
    ...
    if opt.use_depth_hints:
        ...
        depth_hint_reproj_loss = depth_hint_reproj_loss + 1000 * (1 - inputs['depth_hint_mask'])
    else:
    ...
    for scale in opt.scales:
        ...
        reprojection_loss = reprojection_weight * reprojection_loss
        ...
        if opt.use_depth_hints:
            ...
            depth_hint_loss = depth_hint_weight * depth_hint_loss
            # save for logging
            outputs["depth_hint_pixels/{}".format(scale)] = depth_hint_loss_mask
            losses['depth_hint_loss/{}'.format(scale)] = depth_hint_loss

        loss = loss + reprojection_loss + depth_hint_loss
        mean_disp = disp.mean(2, True).mean(3, True)
        norm_disp = disp / (mean_disp + 1e-7)
        smooth_loss = get_smooth_loss(norm_disp, color)
        # 平滑损失乘以可学习权重
        smooth_loss = smoothness_weight * smooth_loss / (2 ** scale)
        # smooth_loss = smooth_loss / (2 ** scale)
        loss = loss + opt.disparity_smoothness * smooth_loss / (2 ** scale)

        total_loss = total_loss + loss / (2 ** scale)
...
def compute_depth_losses(inputs, outputs, print_info=True):
    ...
    depth_pred_scaled = depth_pred * (np.median(depth_gt) / np.median(depth_pred))

    depth_pred = paddle.clip(depth_pred_scaled, min=1e-3, max=80)
...
def compute_depth_losses_val(inputs, outputs, print_info=True):
    """Compute depth metrics, to allow monitoring during training
    ...
    if print_info:
        ...
        print("-> Done!")
    # logger.info("eval")
    # logger.info("  " + ("{:>8} | " * 7).format("abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"))
    # logger.info(("&{: 8.3f}  " * 7).format(*mean_errors.tolist()) + "\\\\")
    # logger.info("-> Done!")
    return mean_errors
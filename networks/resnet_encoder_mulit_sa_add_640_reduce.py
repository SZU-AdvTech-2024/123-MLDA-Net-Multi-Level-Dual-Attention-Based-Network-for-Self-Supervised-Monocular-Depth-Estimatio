...
class ResnetEncoder_multi_sa_add_reduce_640(nn.Layer):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        if num_layers > 34:
            self.num_ch_enc[1:] = self.num_ch_enc[1:] * 4
...
class ResnetEncoder_multi_sa_add_reduce_1024(nn.Layer):
    """module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1):
        super(ResnetEncoder_multi_sa_add_reduce_1024, self).__init__()
        ...
        self.upsample1 = nn.UpsamplingBilinear2D(size=(160, 512))
        self.upsample2 = nn.UpsamplingBilinear2D(size=(80, 256))
        self.upsample3 = nn.UpsamplingBilinear2D(size=(40, 128))
        self.upsample4 = nn.UpsamplingBilinear2D(size=(20, 64))
        self.upsample5 = nn.UpsamplingBilinear2D(size=(10, 32))
...
class ResnetEncoder_multi_sa_add_reduce_1024_res50(nn.Layer):
    """module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1):
        super(ResnetEncoder_multi_sa_add_reduce_1024_res50, self).__init__()

        self.pool = nn.MaxPool2D(2, 2)
        # 64 256 512 1024 2048
        self.anet_2048 = Attention_net(2048, 2048)
        self.anet_1024 = Attention_net(1024, 1024)
        self.anet_512 = Attention_net(512, 512)
        self.anet_256 = Attention_net(256, 256)
        self.anet_64 = Attention_net(64, 64)
        ...
        self.convs20 = nn.Conv2D(3 * num_input_images, 256, 3, 1, 1)
        self.bn20 = nn.BatchNorm2D(256)
        self.convs21 = nn.Conv2D(256, 256, 3, 1, 1)
        self.bn21 = nn.BatchNorm2D(256)

        self.convs30 = nn.Conv2D(3 * num_input_images, 512, 3, 1, 1)
        self.bn30 = nn.BatchNorm2D(512)
        self.convs31 = nn.Conv2D(512, 512, 3, 1, 1)
        self.bn31 = nn.BatchNorm2D(512)

        self.convs40 = nn.Conv2D(3 * num_input_images, 1024, 3, 1, 1)
        self.bn40 = nn.BatchNorm2D(1024)
        self.convs41 = nn.Conv2D(1024, 1024, 3, 1, 1)
        self.bn41 = nn.BatchNorm2D(1024)

        self.convs50 = nn.Conv2D(3 * num_input_images, 2048, 3, 1, 1)
        self.bn50 = nn.BatchNorm2D(2048)
        self.convs51 = nn.Conv2D(2048, 2048, 3, 1, 1)
        self.bn51 = nn.BatchNorm2D(2048)

        self.relu = nn.ReLU()
        # self.adjust_conv_1 = nn.Conv2D(128, 256, 3, 1, 1)
        self.convs64_1 = nn.Conv2D(128, 64, 3, 1, 1)
        self.bn64_1 = nn.BatchNorm2D(64)
        self.convs64_2 = nn.Conv2D(64, 64, 3, 1, 1)
        self.bn64_2 = nn.BatchNorm2D(64)
        ...
        self.convs1024_1 = nn.Conv2D(2048, 1024, 3, 1, 1)
        self.bn1024_1 = nn.BatchNorm2D(1024)
        self.convs1024_2 = nn.Conv2D(1024, 1024, 3, 1, 1)
        self.bn1024_2 = nn.BatchNorm2D(1024)

        self.convs2048_1 = nn.Conv2D(4096, 2048, 3, 1, 1)
        self.bn2048_1 = nn.BatchNorm2D(2048)
        self.convs2048_2 = nn.Conv2D(2048, 2048, 3, 1, 1)
        self.bn2048_2 = nn.BatchNorm2D(2048)
    ...
    def forward(self, input_image):
        ...
        # 在拼接操作后添加1x1卷积层来提升维度
        x_tmp = paddle.concat((x_feature, x_tmp11), 1)
        # x_tmp = self.adjust_conv_1(x_tmp)
        x_tmp = self.relu(self.bn64_1(self.convs64_1(x_tmp)))
        x_tmp = self.relu(self.bn64_2(self.convs64_2(x_tmp)))
        x_tmp = self.anet_64(x_tmp)
        self.features.append(x_tmp)
        ...
        x_feature = self.encoder.layer3(x_feature)
        x_tmp = paddle.concat((x_feature, x_tmp41), 1)
        x_tmp = self.relu(self.bn1024_1(self.convs1024_1(x_tmp)))
        x_tmp = self.relu(self.bn1024_2(self.convs1024_2(x_tmp)))
        x_tmp = self.anet_1024(x_tmp)
        self.features.append(x_tmp)

        x_feature = self.encoder.layer4(x_feature)
        x_tmp = paddle.concat((x_feature, x_tmp51), 1)
        x_tmp = self.relu(self.bn2048_1(self.convs2048_1(x_tmp)))
        x_tmp = self.relu(self.bn2048_2(self.convs2048_2(x_tmp)))
        x_tmp = self.anet_2048(x_tmp)
        self.features.append(x_tmp)

...
from losses.loss import compute_depth_losses, compute_losses, compute_depth_losses_val
...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        # outputs = {**outputs, **self.predict_poses(inputs, features)}
    ...
    def val(self, only_val=False):
        ...
        with paddle.no_grad():
            ...
            for batch_idx, inputs in enumerate(self.val_loader):
                ...
                if "depth_gt" in inputs:
                    # rmse = compute_depth_losses(inputs, outputs, self.logger, print_info=False)
                    mean_errors = compute_depth_losses_val(inputs, outputs, print_info=False)
                    # rmses.append(rmse)
                    rmse = mean_errors[2]
                    abs.append(mean_errors[0])
                    sq_rel.append(mean_errors[1])
                    rmse_log.append(mean_errors[3])
                    a1.append(mean_errors[4])
                    a2.append(mean_errors[5])
                    a3.append(mean_errors[6])
                    rmses.append(rmse)
                    if rmse < rmse_min:
                        rmse_min = rmse
            ...
            abss=np.mean(abs)
            sq_rels = np.mean(sq_rel)
            rmse_logs= np.mean(rmse_log)
            a1s= np.mean(a1)
            a2s= np.mean(a2)
            a3s= np.mean(a3)
            rmse_max = np.max(rmses)
            self.logger.info("val_dateset: rmse_avg={},rmse_min={} ,rmse_max={} ".format(rmse_avg, rmse_min, rmse_max))
            self.logger.info("val_dateset: abs={},sq_rel={} ,rmse = {} , rmse_log={} , a1 = {}, a2={} , a3= {} ".format(abss, sq_rels , rmse_avg, rmse_logs , a1s , a2s , a3s))
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            dis = disp.clone()
            disp = F.interpolate(
                dis, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
            source_scale = 0
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                color_copy = inputs[("color", frame_id, source_scale)].clone()
    ...
    def save_img(self, mode, inputs, outputs):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            ...
            # 处理预测深度图，转换为近蓝远红伪彩色深度图
            pred_depth = normalize_image(outputs[("disp", 0)][j, 0]).detach().cpu().numpy()
            # 减去最小值，将数据范围平移到从0开始（便于后续归一化等操作）
            pred_depth -= pred_depth.min()
            # 归一化到0-1范围（假设原始深度值没有归一化到这个范围，如果已归一化可适当调整）
            pred_depth = pred_depth / (pred_depth.max() - pred_depth.min())
            # 调整范围到0-255（转换为8位表示，类似convertPNG中的处理）
            pred_depth *= 255
            # 使得越近的地方深度值越大（这里进行反转，实现近蓝远红效果）
            # pred_depth = 255 - pred_depth
            # 转换为无符号8位整数类型
            pred_depth = pred_depth.astype(np.uint8)
            # 应用颜色映射，得到近蓝远红的伪彩色深度图
            im_color_pred_depth = cv2.applyColorMap(cv2.convertScaleAbs(pred_depth, alpha=1), cv2.COLORMAP_JET)
            mid_img2save = np.concatenate((input_color, im_color_pred_depth), 0)
            ...
            # 处理disp深度图，转换为近蓝远红伪彩色深度图（原理同pred_depth处理）
            disp = 1 / (inputs['depth_hint'] + 1e-7) * inputs['depth_hint_mask']
            disp = normalize_image(disp[j, 0]).cpu().numpy()
            disp -= disp.min()
            disp = disp / (disp.max() - disp.min())
            disp *= 255
            # disp = 255 - disp
            disp = disp.astype(np.uint8)
            im_color_disp = cv2.applyColorMap(cv2.convertScaleAbs(disp, alpha=1), cv2.COLORMAP_JET)
            
            mid_img2save = np.concatenate((mid_img2save, im_color_disp), 0)
